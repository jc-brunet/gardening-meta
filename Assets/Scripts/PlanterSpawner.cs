...
public class PlanterSpawner : NetworkBehaviour
{
    ...
    public override void OnNetworkSpawn()
    {
        if (IsServer)
        {
            ...
            for (int i = 0; i < _numberMaxX; i++)
            {
                ...
                {
                    GameObject planterPrefab = Instantiate(PlanterPrefab, _newPosition, PlanterPrefab.transform.rotation, this.transform);
                    NetworkObject spawnedPlanter = planterPrefab.GetComponent<NetworkObject>();
                    spawnedPlanter.Spawn();
                    spawnedPlanter.TrySetParent(gameObject, true);
